...
# NOTE: This is the ClickHouse version (legacy) - uses ClickHouse SQL syntax.
...
# SQL query template to insert meta series from S3 Parquet staging file to metaSeries table.
# This query reads from S3 Parquet files using DuckDB's read_parquet() function.
# Uses DuckDB SQL syntax (CASE WHEN, COALESCE, CURRENT_TIMESTAMP).
# The {full_s3_path} placeholder should be replaced with the actual S3 path.
INSERT_META_SERIES_FROM_S3_STAGING_SQL = """
    INSERT INTO metaSeries (
        series_id, series_name, series_code, data_source,
        field_type_id, asset_class_id, sub_asset_class_id, product_type_id,
        data_type_id, structure_type_id, market_segment_id, ticker_source_id,
    ...
    SELECT
        (SELECT COALESCE(MAX(series_id), 0) FROM metaSeries) +
        row_number() OVER (ORDER BY st.series_code) AS series_id,
        st.series_name,
        st.series_code,
        st.data_source,
        -- Resolve field_type: name -> ID
        CASE WHEN st.field_type IS NOT NULL AND st.field_type != '' THEN ft.field_type_id ELSE NULL END AS field_type_id,
        -- Resolve asset_class: name -> ID
        CASE WHEN st.asset_class IS NOT NULL AND st.asset_class != '' THEN ac.asset_class_id ELSE NULL END AS asset_class_id,
        -- Resolve sub_asset_class: name -> ID
        CASE WHEN st.sub_asset_class IS NOT NULL AND st.sub_asset_class != '' THEN sac.sub_asset_class_id ELSE NULL END AS sub_asset_class_id,
        -- Resolve product_type: name -> ID
        CASE WHEN st.product_type IS NOT NULL AND st.product_type != '' THEN pt.product_type_id ELSE NULL END AS product_type_id,
        -- Resolve data_type: name -> ID
        CASE WHEN st.data_type IS NOT NULL AND st.data_type != '' THEN dt.data_type_id ELSE NULL END AS data_type_id,
        -- Resolve structure_type: name -> ID
        CASE WHEN st.structure_type IS NOT NULL AND st.structure_type != '' THEN stt.structure_type_id ELSE NULL END AS structure_type_id,
        -- Resolve market_segment: name -> ID
        CASE WHEN st.market_segment IS NOT NULL AND st.market_segment != '' THEN ms.market_segment_id ELSE NULL END AS market_segment_id,
        -- Resolve ticker_source: name -> ID
        CASE WHEN st.ticker_source IS NOT NULL AND st.ticker_source != '' THEN ts.ticker_source_id ELSE NULL END AS ticker_source_id,
        st.ticker,
        -- Resolve region: name -> ID
        CASE WHEN st.region IS NOT NULL AND st.region != '' THEN r.region_id ELSE NULL END AS region_id,
        -- Resolve currency: code -> ID
        CASE WHEN st.currency IS NOT NULL AND st.currency != '' THEN c.currency_id ELSE NULL END AS currency_id,
        -- Resolve term: name -> ID
        CASE WHEN st.term IS NOT NULL AND st.term != '' THEN t.term_id ELSE NULL END AS term_id,
        -- Resolve tenor: code -> ID
        CASE WHEN st.tenor IS NOT NULL AND st.tenor != '' THEN tn.tenor_id ELSE NULL END AS tenor_id,
        -- Resolve country: code -> ID
        CASE WHEN st.country IS NOT NULL AND st.country != '' THEN co.country_id ELSE NULL END AS country_id,
        st.calculation_formula,
        st.description,
        -- Parse is_active: convert string to INTEGER (default to 1 if not provided)
        CASE 
            WHEN st.is_active IS NULL OR st.is_active = '' THEN 1
            WHEN LOWER(st.is_active) IN ('1', 'true', 'yes', 'y', 'active') THEN 1
            ELSE 0
        END AS is_active,
        CURRENT_TIMESTAMP AS created_at,
        CURRENT_TIMESTAMP AS updated_at,
        'csv_loader' AS created_by
    FROM read_parquet('{full_s3_path}') st
    LEFT JOIN fieldTypeLookup ft ON st.field_type = ft.field_type_name
    LEFT JOIN assetClassLookup ac ON st.asset_class = ac.asset_class_name
    LEFT JOIN productTypeLookup pt ON st.product_type = pt.product_type_name
    LEFT JOIN dataTypeLookup dt ON st.data_type = dt.data_type_name
