...
def failure_message_fn(context: RunFailureSensorContext) -> str:
    ...
    try:
        ...
        logger.info(f"Generating Teams failure notification for job '{job_name}', run_id: {run_id}")
        ...
        logger.error(
            ...
            f"run_id: {context.dagster_run.run_id}. Error: {e!s}",
            exc_info=True,
        )
...
def success_message_fn(context: RunStatusSensorContext) -> str:
    ...
    try:
        ...
        logger.info(f"Generating Teams success notification for job '{job_name}', run_id: {run_id}")
